...
#define DRV_USART_SERCOM0_TX_INT_ENABLE()       SERCOM0_REGS->USART_INT.SERCOM_INTENSET = SERCOM_USART_INT_INTENSET_DRE_Msk
...
void UART_PPSInit(_DRV_UART_Index_T idx)
{
    ...
    CFG_REGS->CFG_CFGCON1 |= direct_set[idx] ;
...
#ifdef APP_UART_TX_INT_SUPPORT
        SERCOM0_USART_WriteNotificationEnable(p_plibSet->usartWrNotificationEnabled, p_plibSet->usartWrNotifyPersistently);
        SERCOM0_USART_WriteCallbackRegister(p_plibSet->usartWriteCb, (uint32_t)p_dev);
#endif
