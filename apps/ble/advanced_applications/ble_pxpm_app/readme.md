...
| APP_PXP_IND_STA_CONNECTED      | Green LED flashes twice every 1.5 seconds. (On: 50 ms, Off: 150 ms).  |
| APP_PXP_IND_STA_WITH_BOND_SCAN | Green LED flashes twice every 3 seconds. (On: 50 ms, Off: 50 ms).     |
| APP_PXPM_STATE_SCAN            | Green LED flashes onec every 3 seconds. (On: 50 ms, Off: 2950 ms).    |


- RGB LEDs indicate the alert state according to the following table:

| APP Alert Level             |  PXPR LED behavior                           |
| :-------------------------- | :------------------------------------------ |
| BLE_PXPR_ALERT_LEVEL_NO     | Green LED keep lighting for 5 seconds.        |
| BLE_PXPR_ALERT_LEVEL_MILD   | Green LED blinks once every second continuously for 5 seconds.      |
| BLE_PXPR_ALERT_LEVEL_HIGH   | Green LED blinks five times every second continuously for 5 seconds.    |
...
5. When the PXPR B is within 10cm of the PXPM, the **BLE_PXPR_ALERT_LEVEL_NO** alert level can be observed through Green led.
6. PXPR B moves away from PXPM over 3m, the **BLE_PXPR_ALERT_LEVEL_MILD** alert level can be observed through PXPR Green led.
7. PXPR B keep moves away from PXPM over 30m, the **BLE_PXPR_ALERT_LEVEL_HIGH** alert level can be observed through the PXPR B Green LED.
8. Disconnect the PXPR B. The PXPM will enter **APP_PXP_IND_STA_WITH_BOND_SCAN**. The alert level set in step 4 of the link loss service will be displayed as **BLE_PXPR_ALERT_LEVEL_HIGH** and can be observed through the PXPR B Green LED.
	- Power off the PXPM by "unplug USB cable".
	- keep away from the PXPR until disconnected.
9. Once the PXPR B is reconnected, it will enter **APP_PXP_IND_STA_CONNECTED** and the Green LED alert situation will be re-indicated as the corresponding alert level specified by the path loss table.
